...
const TechDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [statusData, recordsData, leavesData] = await Promise.allSettled([
        attendanceService.getTodayStatus(userId),
        attendanceService.getRecords({ employeeId: userId, page: 1, limit: 10 }),
        leaveService.getLeavesByEmployee(userId),
      ]);
    ...
    <div>
      ...
      <div className="content-grid">
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: '0.8125rem', color: '#5f6368', marginBottom: 4 }}>Employee Code</div>
              <div style={{ fontSize: '1rem', fontWeight: 600 }}>{user?.employeeCode || '—'}</div>
            </div>
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: '0.8125rem', color: '#5f6368', marginBottom: 4 }}>Department</div>
              <div style={{ fontSize: '1rem', fontWeight: 600 }}>{user?.department || '—'}</div>
            </div>
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: '0.8125rem', color: '#5f6368', marginBottom: 4 }}>Role</div>
